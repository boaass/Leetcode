...
class Solution(object):
    def lemonadeChange(self, bills):
        ...
        """
        rest_money_sum = 0
        rest_money_dict = {5: 0, 10: 0, 20: 0}
        for v in bills:
            print rest_money_sum
            print rest_money_dict

            if v == 5:
                rest_money_sum += v
                rest_money_dict[v] += 1
            elif v == 10:
                if rest_money_sum >= 5 and rest_money_dict[5] >= 1:
                    rest_money_sum += 5
                    rest_money_dict[5] -= 1
                    rest_money_dict[10] += 1
                else:
                    return False
            elif v == 20:
                if rest_money_sum >= 15:
                    if rest_money_dict[5] >= 1 and rest_money_dict[10] >= 1:
                        rest_money_sum += 5
                        rest_money_dict[20] += 1
                        rest_money_dict[5] -= 1
                        rest_money_dict[10] -= 1
                    elif rest_money_dict[5] >= 3:
                        rest_money_sum += 5
                        rest_money_dict[20] += 1
                        rest_money_dict[5] -= 3
                    else:
