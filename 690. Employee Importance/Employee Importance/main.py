...
# importance value 15, 10 and 5, respectively. Then employee 1 has a data structure like [1, 15, [2]], and employee 2
# has [2, 10, [3]], and employee 3 has [3, 5, []]. Note that although employee 3 is also a subordinate of employee 1,
...
class Solution(object):
    def getImportance(self, employees, id):
        ...
        for employe in employees:
            employe_dict[employe.id] = employe

        total = 0
        queue = [employe_dict[id]]
        while len(queue) > 0:
            cur_employe = queue.pop(0)
            queue += [employe_dict[id] for id in cur_employe.subordinates]
            total += cur_employe.importance
...
if __name__ == '__main__':
    ...
    solution = Solution()
    r = solution.getImportance(employees, id)
